...
func NewCore(
	...
	core := &Core{
		...
		Commitor:       NewCommitor(CallBack, pool),
...
func (c *Core) generateBlock(epoch int64) *ConsensusBlock {
...
func (c *Core) handleOutput(epoch int64, leader core.NodeID) error {
	...
	if cbc.BlockHash != nil {
		if block, err := c.getConsensusBlock(*cbc.BlockHash); err != nil {
			logger.Warn.Println(err)
			c.Commitor.Commit(epoch, leader, nil)
		} else {
			if ok := c.verifyConsensusBlock(block); !ok {
				logger.Error.Printf("handle proposal and want to commit but error and try to retriver Author %d Epoch %d lenof Reference %d\n", leader, epoch, len(block.PayLoads))
			} else {
			...
			if block.Proposer != c.Name {
				...
				if temp != nil {
					if block, err := c.getConsensusBlock(*temp); err == nil && block != nil {
...
func (c *Core) handleLoopBack(blockhash crypto.Digest) error {
	if block, err := c.getConsensusBlock(blockhash); err != nil {
		...
		proposal, _ := NewProposal(block.Proposer, block, block.Epoch, c.SigService)

		if c.Commitor.CommitLeader(proposal.Epoch) == proposal.Author {
